...
st.set_page_config(page_title="TUAN PHONG V19.6", layout="centered")
...
def calculate_master(use_ai):
    ...
    # TRUNG BÌNH CỘNG LÀM RÀO CHẮN (Ý mày: Không được đưa xuống dưới TBC)
    avg_rank = (r1 + r2 + r3 + r4) / 4
    
    # Tính toán Đối trọng AI
    ...
    # ÉP LOGIC: Điểm AI chỉ được phép đưa số LÊN, không được dìm xuống dưới TBC
...
db = st.session_state.db
st.markdown("<h3 style='text-align: center;'>🛡️ TUAN PHONG V19.6 MASTER</h3>", unsafe_allow_html=True)
