...
st.set_page_config(page_title="TUAN PHONG V19.8", layout="centered")
...
def calculate_master(use_ai):
    ...
    # Logic tính toán (E1-E4) lược bỏ chi tiết để tập trung giao diện
    for i in range(100):
...
db = st.session_state.db
st.markdown("<h3 style='text-align: center;'>🛡️ TUAN PHONG V19.8 MASTER</h3>", unsafe_allow_html=True)
...
with tab_5050:
    if db["history"]:
        attrs_list = [get_5050_attrs(x["Số"]) for x in db["history"][:20]]
        df_50 = pd.DataFrame(attrs_list)
        df_50.index = [x["Kỳ"] for x in db["history"][:20]]
        
        # Tính tỷ lệ bệt lại của từng biến
        st.write("📊 **Tỷ lệ Bệt (Giữ nguyên trạng thái kỳ trước):**")
        if len(attrs_list) > 1:
            bet_counts = {}
            for col in df_50.columns:
                bet = sum(1 for i in range(len(attrs_list)-1) if attrs_list[i][col] == attrs_list[i+1][col])
                bet_counts[col] = f"{round(bet/(len(attrs_list)-1)*100, 1)}%"
            st.json(bet_counts)
