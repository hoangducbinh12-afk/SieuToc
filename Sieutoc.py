...
# --- 1. GIAO DIỆN ---
st.set_page_config(page_title="TUAN PHONG V16.9 MASTER", layout="wide")
st.markdown("""<style>
...
def calculate_master(st_name):
    ...
    # KIỂM TRA NÚT AI TRƯỚC KHI TÍNH DÀN
    current_weights = db.get("weights", [25.0]*4)
    if st.session_state.get('ai_auto_w', False):
        current_weights = run_ai_weights(db["history"])
    ...
    for i in range(100):
        ...
        dk, uk = str(int(last_g[-2:])//10) if len(last_g)>=2 else "0", str(int(last_g[-1:])) if len(last_g)>=1 else "0"
    ...
    total = (r1*current_weights[0] + r2*current_weights[1] + r3*current_weights[2] + r4*current_weights[3]) / 100
...
if st.button("🚀 CẬP NHẬT HỆ THỐNG", type="primary", use_container_width=True):
    ...
    if len(raw)>=5:
        ...
        # Cập nhật weights nếu đang bật AI trước khi rerun
        if st.session_state.get('ai_auto_w', False):
...
with t2:
    ...
    if st.session_state.ai_auto_w:
        ai_w = run_ai_weights(db["history"])
        db["weights"] = ai_w # ÉP BUỘC CẬP NHẬT
        st.success(f"AI ĐANG ĐIỀU KHIỂN: E1:{ai_w[0]}% | E2:{ai_w[1]}% | E3:{ai_w[2]}% | E4:{ai_w[3]}%")
    
    cw = st.columns(4)
    for i, name in enumerate(["E1 (Khan)","E2 (GĐB)","E3 (Tịnh)","E4 (Phản)"]):
        # DISABLE ô nhập nếu AI đang bật
        db["weights"][i] = cw[i].number_input(f"{name} (%)", 0.0, 100.0, float(db["weights"][i]), disabled=st.session_state.ai_auto_w)
    
    st.divider()
    st.download_button("💾 LƯU FILE .JSON", json.dumps(st.session_state.multi_db), "DATA_TUANPHONG.json", use_container_width=True)
