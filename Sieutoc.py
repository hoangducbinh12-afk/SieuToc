...
st.set_page_config(page_title="TUAN PHONG V12.2 MULTI", layout="wide")
st.markdown("""
    <style>
    .main-box { 
...
# --- HÀM KHỞI TẠO BỘ NHỚ TRỐNG CHO MỘT ĐÀI MỚI ---
...
# --- KHỞI TẠO STATE HỆ THỐNG ĐA ĐÀI ---
if 'multi_db' not in st.session_state:
    st.session_state.multi_db = {
        "MB": create_blank_station(),       # Đài Miền Bắc mặc định
        "BINH_DUONG": create_blank_station(),# Đài Bình Dương mẫu
        "VINH_LONG": create_blank_station()  # Đài Vĩnh Long mẫu
    }
if 'current_station' not in st.session_state:
    st.session_state.current_station = "MB"

# --- THUẬT TOÁN BẢN ĐỒ PHẢN XẠ SỐ (THEO ĐÀI HÌNH) ---
def rebuild_reflex_memory_v122(st_name):
...
def update_ai_weights_v122(st_name):
    st_db = st.session_state.multi_db[st_name]
...
# --- ENGINE TÍNH TOÁN THEO BIÊN ĐÀI ĐÃ CHỌN ---
def calculate_master_v122(st_name):
    st_db = st.session_state.multi_db[st_name]
    ...
    if not st_db.get("reflex_memory") or current_num not in st_db["reflex_memory"]:
        rebuild_reflex_memory_v122(st_name)
    ...
    for i in range(100):
        ...
        e1[i] = st_db["dau"][d] + st_db["duoi"][u] + st_db["tong"][t] + st_db["hieu"][h] + \
                ...
                st_db["bo"][b_idx] + st_db["giap"][find_idx(i, GIAP_12)] + \
                st_db["dang5"][find_idx(i, DANG_5)] + st_db["cl4"][find_idx(i, CL_4)] + st_db["bt4"][find_idx(i, BT_4)] + \
        ...
        if ref_map:
            e4[i] = 1000 - ((ref_map["dau"][d] + ref_map["duoi"][u] + ref_map["tong"][t] + ref_map["bo"][b_idx]) * 15)
        else:
            e4[i] = 1000
...
def find_rank_unique_v122(df, target, col, asc=True):
...
def process_multi_update():
    ...
    df_old = calculate_master_v122(st_name)
    r_ai = find_rank_unique_v122(df_old, target_str, "DIEM_TONG", asc=True)
    r_e1 = find_rank_unique_v122(df_old, target_str, "E1", asc=True)
    r_e2 = find_rank_unique_v122(df_old, target_str, "E2", asc=True)
    r_e3 = find_rank_unique_v122(df_old, target_str, "E3", asc=False)
    r_e4 = find_rank_unique_v122(df_old, target_str, "E4", asc=True)
    ...
    # [Nuôi điểm Khan & Tịnh tiến cho riêng đài được chọn ở đây...]
    ...
    st_db["ky_quay"] += 1
    rebuild_reflex_memory_v122(st_name)
    st_db["weights"] = update_ai_weights_v122(st_name)
    st.rerun()

# --- 9. GIAO DIỆN ĐIỀU KHIỂN ĐA ĐÀI V12.2 ---
st.title("🛡️ COMMANDER MULTI-STATION V12.2")

with st.sidebar:
    st.header("🏢 TRẠM ĐIỀU HÀNH ĐÀI")
    # THANH CHỌN ĐÀI PHÂN TÁCH BIÊN DỮ LIỆU
    ...
    # Nút thêm nhanh một tên đài mới bất kỳ vào hệ thống
    new_st = st.text_input("➕ Thêm tên đài mới (VIẾT LIỀN KHÔNG DẤU):")
    if st.button("KÍCH HOẠT ĐÀI MỚI") and new_st:
        if new_st not in st.session_state.multi_db:
            st.session_state.multi_db[new_st] = create_blank_station()
            st.rerun()
            
    st.divider()
    up = st.file_uploader("📥 Nạp File (.Json) Toàn Bộ Đài:", type="json")
    if up:
...
# Giao diện chính nhập số theo đài đã chọn
current_st = st.session_state.current_station
current_db = st.session_state.multi_db[current_st]

st.info(f"🛰️ Hệ thống đang tải bộ nhớ của đài: **[{current_st}]** (Kỳ thứ: {current_db['ky_quay']})")

c1, c2, c3 = st.columns([1.5, 1.5, 1])
with c1: st.text_input(f"Nhập GĐB Đài {current_st}:", value="67294", key="gdb_in")
with c2: st.text_input("Ngày Quay:", datetime.now().strftime("%d%m%Y"), key="date_in")
with c3: st.toggle("MA TRẬN ROOT", value=current_db["use_root"], key="use_root")

st.button("🚀 CẬP NHẬT RIÊNG CHO ĐÀI NÀY", on_click=process_multi_update, type="primary", use_container_width=True)

# THỰC THI HIỂN THỊ DÀN SỐ CHỮ BÉ THEO ĐÀI CỤ THỂ
df_master = calculate_master_v122(current_st)
active_w = update_ai_weights_v122(current_st)

t1, t2, t3 = st.tabs(["🎯 DÀN AI THEO ĐÀI", "⚖️ ĐỐI TRỌNG AI", "📋 NHẬT KÝ ĐÀI"])
with t1:
    n1 = st.number_input("Số quân:", 1, 100, 51)
    danh_sach = df_master.sort_values("DIEM_TONG", ascending=True)["SO"].tolist()
    st.markdown(f"<div class='main-box'>{' '.join(danh_sach[:n1])}</div>", unsafe_allow_html=True)
with t2:
    st.table(pd.DataFrame({"Engine": [f"E{i}" for i in range(1,5)], "Tỷ trọng (%)": [round(x,1) for x in active_w]}).set_index("Engine").T)
    st.download_button("💾 XUẤT FILE JSON TOÀN BỘ ĐÀI", json.dumps(st.session_state.multi_db), "MULTI_STATION_DATA.json", use_container_width=True)
with t3:
    if current_db["history"]: st.table(pd.DataFrame(current_db["history"])[['Kỳ', 'GĐB', 'Số', 'Rank_AI', 'Rank_E1', 'Rank_E2', 'Rank_E3', 'Rank_E4']])